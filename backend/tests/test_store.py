...
from mia_dpp.config import REPOSITORY_ROOT, Settings
...
def test_default_runtime_state_is_not_stored_in_temporary_files(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("MIA_THREAD_STORE_PATH", raising=False)
    monkeypatch.delenv("MIA_WORKSPACE_ROOT", raising=False)

    settings = Settings(_env_file=None)

    assert settings.thread_store_path == REPOSITORY_ROOT / ".mia-data/mia-agent.sqlite3"
    assert settings.workspace_root == REPOSITORY_ROOT / ".mia-data/workspaces"
