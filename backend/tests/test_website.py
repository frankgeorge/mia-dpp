...
async def ingest(
    ...
    *,
    ...
    mapping = await DeterministicWebsiteMapper(repository, index).propose(extraction.evidence)
    work = ProductWork(
...
def test_repeated_technical_concepts_keep_component_context() -> None:
    ...
    assert [item.context_path for item in protection] == [("Probe",), ("Control unit",)]
