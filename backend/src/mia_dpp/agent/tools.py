...
async def extract_product_page(
    ...
    sources_by_id = {item.id: item for item in work.acquired_sources}
    sources_by_id.update({item.id: item for item in package.acquired_sources})
    work.acquired_sources = tuple(sources_by_id.values())
    ...
    source_artifact = ctx.deps.store.write_json(
        ...
        "source.json",
        {
            ...
            "sources": [
                ...
                for item in package.acquired_sources
            ],
        },
    ...
    raw_artifacts = tuple(
        ctx.deps.store.write_bytes(
            ctx.deps.state.thread_id,
            ArtifactKind.RAW,
            "rendered-source.html",
            item.rendered_html.encode("utf-8"),
    ...
    evidence_artifact = ctx.deps.store.write_json(
        ...
        product_id=resolved_id,
        source_url=source_url,
        derived_from=(source_artifact.id, *(item.id for item in raw_artifacts)),
    )
    work.artifact_ids = (
        *work.artifact_ids,
        source_artifact.id,
        *(item.id for item in raw_artifacts),
...
async def map_product_evidence(
    ...
    if work.mapping_result is not None:
        ...
        } | set(work.mapping_result.unmatched_evidence_ids) | set(
            work.mapping_result.irrelevant_evidence_ids
        ) | set(work.mapping_result.rejected_evidence_ids)
    ...
    if ctx.deps.semantic_mapper is None:
        return ToolObservation(
            outcome="semantic_mapper_required",
            summary="Configure the semantic model before starting a complete mapping cycle.",
            count=0,
        )
    started = time.monotonic()
    ...
    deterministic = await DeterministicWebsiteMapper(ctx.deps.templates, index).propose(
        package.evidence
    )
    company = ctx.deps.state.selected_company
    knowledge = tuple(
    ...
    semantic_run = await ctx.deps.semantic_mapper.map(
        package,
        index,
        deterministic,
        reviewed_knowledge=knowledge,
    )
    mapping_result = ctx.deps.mapping_review.apply_semantic_run(
        package,
        deterministic,
        index,
        semantic_run,
    ...
    work.mapping_cycle_id = ctx.deps.mapping_review.cycle_id(package, index, mapping_result)
    work.pending_reviews = (
        ()
        if work.mapping_cycle_id in work.confirmed_mapping_cycle_ids
        else ctx.deps.mapping_review.complete_review(package, mapping_result, index)
    )
    ctx.deps.state.products[product_id] = work
    lean_evidence_artifact = ctx.deps.store.write_json(
        ctx.deps.state.thread_id,
        ArtifactKind.MAPPING,
        "semantic-evidence.json",
        [item.model_dump(mode="json") for item in semantic_run.evidence],
        created_by="map_product_evidence",
        product_id=product_id,
        derived_from=work.artifact_ids,
    )
    lean_targets_artifact = ctx.deps.store.write_json(
        ctx.deps.state.thread_id,
        ArtifactKind.MAPPING,
        "semantic-targets.json",
        [item.model_dump(mode="json") for item in semantic_run.targets],
        created_by="map_product_evidence",
        product_id=product_id,
    )
    semantic_result_artifact = ctx.deps.store.write_json(
        ctx.deps.state.thread_id,
        ArtifactKind.MAPPING,
        "batch-semantic-result.json",
        {
            "result": semantic_run.result.model_dump(mode="json"),
            "metrics": semantic_run.metrics.model_dump(mode="json"),
        },
        created_by="map_product_evidence",
        product_id=product_id,
        derived_from=(lean_evidence_artifact.id, lean_targets_artifact.id),
    )
    mapping_artifact = ctx.deps.store.write_json(
        ...
        created_by="map_product_evidence",
        product_id=product_id,
        derived_from=(semantic_result_artifact.id,),
    )
    ...
    work.artifact_ids = (
        *work.artifact_ids,
        lean_evidence_artifact.id,
        lean_targets_artifact.id,
        semantic_result_artifact.id,
        mapping_artifact.id,
        coverage_artifact.id,
    )
    ...
    ctx.deps.add_event(
        ...
        metadata={
            ...
            "semanticEvidenceCount": semantic_run.metrics.evidence_count,
            "semanticTargetCount": semantic_run.metrics.target_count,
            "semanticInputTokens": semantic_run.metrics.input_tokens,
            "semanticOutputTokens": semantic_run.metrics.output_tokens,
        },
    )
    return ToolObservation(
        ...
        count=len(mapping_result.outcomes),
        identifiers=(product_id,),
    )
...
async def build_product_aas(
    ...
    blocking = tuple(
        ...
        and requirements[item.requirement_id].kind.value == "value"
