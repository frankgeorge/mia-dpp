...
class CoverageAnalyzer:
    ...
    def analyze(
        ...
        *,
        ...
        self._add_mapping_matches(
            inventory,
            evidence_by_id,
            mapping_result,
            matches,
        )
        self._add_label_matches(package.evidence, inventory.requirements, matches)

        candidate_concepts: dict[str, set[str]] = defaultdict(set)
        for (requirement_id, evidence_id), match in matches.items():
            if match.rank == 1:
                candidate_concepts[evidence_id].add(
                    self._concept_key(requirements_by_id[requirement_id])
                )
        ...
        statistics = self._statistics(inventory, coverage, len(analyzed_ids), len(used))
    ...
    def _add_label_matches(
        ...
        for record in evidence:
            source_label = cls._normalize(record.source_label or record.predicate)
            alias_targets = _STRONG_ALIASES.get(source_label, frozenset())
            for requirement in fixed_values:
                if not cls._value_compatible(record, requirement):
                    continue
                target_label = cls._normalize(requirement.id_short or "")
                ...
                elif cls._plausible_label(source_label, target_label):
                    cls._record(
                        matches,
                        requirement.id,
                        record.id,
    ...
    def _coverage_for(
        ...
        strong = [
            (evidence_id, match) for evidence_id, match in requirement_matches if match.rank == 2
        ]
        if strong:
            strong_ids = tuple(evidence_id for evidence_id, _ in strong)
            distinct_values = {
                CoverageAnalyzer._normalized_value(by_id[evidence_id].value)
        ...
        if ambiguous:
            return RequirementCoverage(
                ...
                explanation=(
                    "Multiple values or target meanings compete, so no deterministic choice "
                    "is safe."
                ),
            )
    ...
    def _statistics(
        ...
        def count(required: bool, status: CoverageStatus) -> int:
            return sum(
                1
                for item in coverage
                if requirement_by_id[item.requirement_id].required is required
    ...
    def _value_compatible(evidence: EvidenceRecord, requirement: Requirement) -> bool:
        ...
        if evidence.unit and requirement.unit:
            if CoverageAnalyzer._normalize(evidence.unit) != CoverageAnalyzer._normalize(
                requirement.unit
            ):
                return False
    ...
    def _concept_key(requirement: Requirement) -> str:
        ...
        return CoverageAnalyzer._normalize(label)
